defmodule Helios.Endpoint.Instrument do
  @moduledoc false
  ...
  def definstrument(otp_app, endpoint) do
    ...
    quote bind_quoted: [app_instrumenters: app_instrumenters] do
      ...
      defmacro instrument(event, runtime \\ Macro.escape(%{}), fun) do
        compile = Macro.escape(Helios.Endpoint.Instrument.strip_caller(__CALLER__))
      ...
      for {event, instrumenters} <- app_instrumenters do
        def instrument(unquote(event), var!(compile), var!(runtime), fun)
          ...
          unquote(Helios.Endpoint.Instrument.compile_start_callbacks(event, instrumenters))
          start = :erlang.monotonic_time
          try do
            fun.()
          after
            var!(diff) = :erlang.monotonic_time - start
            unquote(Helios.Endpoint.Instrument.compile_stop_callbacks(event, instrumenters))
          end
        end
      end
  ...
  defp app_instrumenters(otp_app, endpoint) do
    ...
    events_to_instrumenters([Helios.Plugs.Logger | instrumenters])
  end
