defmodule Helios.CodeReloader.Server do
  @moduledoc false
  use GenServer

  require Logger
  alias Helios.CodeReloader.Proxy
      ...
      case :file.read_link(priv_path) do
        ...
        {:error, _} ->
          if can_symlink?() do
            ...
            Logger.warn "Helios is unable to create symlinks. Helios' code reloader will run " <>
  ...
  defp os_symlink({:win32, _}),
    do: " On Windows, the lack of symlinks may even cause empty assets to be served. " <>
        ...
        "once with \"Run as Administrator\" and then running your Helios application."
  defp os_symlink(_),
    do: ""
